...
public class PageRankDriver {
	...
	static void diff(String input1, String input2, String output, int reducers)
		...
		{
			System.out.println("Diff Part 1 Complete\n,Part 2 Started\n");
	...
	static void join(String ranksInput, String namesInput, String output, int reducers) throws Exception {
		...
		System.out.print(job.waitForCompletion(true) ? "Join Job Completed\n" : "Join Job Error");
	...
	public static void composite(String input, String output, String interim1,
		 ...
		 * Put your name here
		 */
		System.out.println("Durga Prasad reddy Katam (20009043)");
		...
		/*
		 * The two branches here extract (vertex, rank) pairs from the result of the last iteration
		...
		{
			//  Modify this to output (vertex name, page rank) pairs instead of (vertex id, page rank)
				...
				if (file.getPath().getName().startsWith("part-r-00")) {
					...
					while (i <= 10 && diffcontent != null) // Adds only 10 values max. per file
					{
						...
						if (diffcontent != null) // Adds new value as long as value exists
						{
							...
							String[] rankandname = parts[1].split(" ");
							double rank = Double.parseDouble(rankandname[0]);
